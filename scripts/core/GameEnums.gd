...
enum ResourceType {
	...
	ENERGY,              ## Capacity pool: producers grant it once at construction, consumers reserve a flat share once at construction, both reversed on ruin/demolish (BuildingCapacityAllocator).
	POPULATION,          ## Capacity pool, same mechanics as ENERGY: housing grants it, facilities/units are meant to reserve a flat share against it, refunded on demolition/death. Only housing wires into it so far — non-housing building/unit Pop costs land with the Building/Unit tree rework (todo.md).
