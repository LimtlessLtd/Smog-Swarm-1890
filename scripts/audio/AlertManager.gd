...
## The audio/auto-pause alert system — the consumer that turns
## EventManager's typed queue into something the player actually notices: a
## synthesized chime (AlertTones) and — the real point of it, for a game
## that spends most of its time running at up to 1000x — auto-pausing the
## simulation (TickManager.set_speed_index(0), the same "0x" tier the speed
## buttons use) so nothing serious rolls past unnoticed while the player's
...
## backs which event.
##
## Desktop OS-level notifications are NOT implemented — Godot has no
## cross-platform "toast in the system tray" API without a third-party
## plugin, and this project has no such dependency anywhere else. In-app
## is the whole notification surface (the audio + auto-pause here, the
## visible toast in MainHUD, the pulsing map marker in
## StrategicOverlayManager).
##
## Sunset/sunrise chimes ("Church bell toll"/"rooster call" at the 2-minute
## mark) poll TimeCycleManager once a second — no "N seconds before a phase
## flip" signal exists to hang a one-shot callback off directly. Each
## direction has its own "armed" flag so the chime fires exactly once per
## approach: crossing into Day re-arms the sunset warning (there's a fresh
## nightfall ahead to warn about), crossing into Night re-arms the sunrise
## warning, symmetrically.
##
## Real-time cooldown on top of that: the "once per approach" gate above is
## correct in GAME time, but at high TickManager speeds a whole 40-minute
## day compresses into a few real seconds (2.4s at 1000x) — sunset and
## sunrise then legitimately alternate fast enough to sound like a
## near-continuous chime. _try_play_chime() additionally floors how often a
## chime can actually SOUND to once per CHIME_MIN_REAL_INTERVAL_MS,
## measured against Time.get_ticks_msec() (real OS time, deliberately NOT
## TickManager/game time — unaffected by Engine.time_scale, unlike
## everything else in this class) — a legitimate new crossing while still
## on cooldown just plays no sound at all rather than queuing or delaying
## one; the "armed" flags above are untouched by this, so the very next
## crossing after cooldown still gets checked fresh. Scoped to these two
## chimes only — EventManager-driven alert tones (_on_event_raised(),
## below) are a different, higher-stakes category (they gate auto-pause)
## and stay unthrottled.

@export var event_manager_path: NodePath

const NIGHTFALL_WARNING_SECONDS: float = 120.0
const PHASE_CHECK_INTERVAL_SECONDS: float = 1.0
const CHIME_MIN_REAL_INTERVAL_MS: int = 30000  ## Day/night alarms shouldn't sound more than once every 30 real-world seconds, however fast the in-game clock is running.
...
## "Remove all the sounds for now" (user feedback) — every AudioStreamPlayer
...
## ever set .bus to anything else. Muting that one bus here, once, at boot
## silences all of them centrally without deleting AlertTones' own
...
## produces no audible sound. Deliberately NOT deleting the code: flip this
## back off and every existing call site works again unchanged. See
## BackgroundExecutionManager's own doc comment for why bus-muting as a
## concept already has precedent in this project (it explicitly does NOT
## do this while unfocused, for a different reason — simulation/audio
## staying "live" in the background — this is an unconditional, always-on
## mute instead, not tied to focus).
