...
##
## Per-unit upgrade nodes (design_doc.md §4's "2 research upgrades per unit
## type") are now real — the "smaller, separate node, layered on once the
## unit-tier system grows" this comment used to defer. They live in
## UnitUpgradeCatalog (which owns their effect table as well as their tech
## metadata) and are merged into this tree by _build_definitions() below.
##
## "Advanced building variant" is still deliberately unrepresented: no such
## variant building exists anywhere in the project to actually unlock.
## GameEnums.TechUnlockType.BUILDING_VARIANT remains a documented hook.
...
static func _build_definitions() -> Array[TechDefinition]:
	var definitions: Array[TechDefinition] = _core_definitions()
	# The 36 per-unit upgrade nodes (2 per roster entry), generated by
	# UnitUpgradeCatalog and merged in here rather than transcribed above —
	# that class owns both their tech metadata and their effect, so there's
	# one table to keep in step with the roster instead of two. They research
	# through exactly the same TechManager path as everything else; nothing
	# about the research flow is special-cased for them.
	definitions.append_array(UnitUpgradeCatalog.get_tech_definitions())
	return definitions

static func _core_definitions() -> Array[TechDefinition]:
	return [
