...
## One fixed simulation step through every per-frame gameplay manager.
...
## A passable land hex at exactly `distance` from the start, preferring the
## most (or least) populated one so scenarios hit a real threat, not moorland.
func hex_at_distance(distance: int, most_populated: bool = true) -> Vector2i:
	var best := start_hex
	var best_capacity := -1
	var hex_grid_map: HexGridMap = main.get_node("WorldRoot/HexGridMap")
