...
## 5.6), and kill_count (Phase 5.7) are the exceptions saved alongside them
## — genuinely mutable per-instance state that can't be re-derived from the
## definition. The in-flight movement path itself (UnitInstance.path) is
## NOT saved, same as Horde.path — cheap for UnitOrderController to replan
## from scratch, and a patrol always restarts its loop from leg 0 after a
## load rather than resuming mid-leg (UnitInstance.patrol_target_index
## isn't saved either).
...
func _init(p_unit_type: GameEnums.UnitType = GameEnums.UnitType.TRUNCHEONEER, p_hex_coord: Vector2i = Vector2i.ZERO, p_id: int = 0, p_current_hp: float = 0.0, p_order: GameEnums.UnitOrderType = GameEnums.UnitOrderType.HOLD, p_move_target: Vector2i = Vector2i.ZERO, p_patrol_waypoints: Array[Vector2i] = [], p_kill_count: int = 0) -> void:
