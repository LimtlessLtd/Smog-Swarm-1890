...
- [ ] **4.1 Chokepoint Defensive Building (`BuildingSystem.gd`)**
  ...
  - [ ] Wall progression: Wooden Walls -> Brick Walls -> Concrete Walls (tier unlocked via Phase 2.9's Tech Tree) but each wall segment must be upgraded individually and costs resources to upgrade, but 50% less than building the wall from scratch.
  ...
  - [ ] Retain legacy inner walls as fallback bulkheads during breach events — a breached outer segment doesn't expose the district directly depending on how the player has built their settlements/cities, it exposes it through that specific gap to whatever's immediately behind it (see 4.2's expanded siege-cascade note).
...
- [ ] **5.1 40-Minute Day/Night Simulation (`TimeCycleManager.gd`)**
  - [ ] **Day Phase (20 Minutes):**
    ...
    - [ ] Military units get increased movement speed and damage.
  ...
  - [ ] Date counter, showing how long has elapsed since emerging from Manchesters self imposed quarantine to save itself from The Swarm (January 1st 1890).
...
- [ ] **6.1 Background Play UI & HUD Design (`MainHUD.tscn`)**
  - [ ] Day/Night Phase Clock indicator with countdown timer (e.g., "Nightfall in 04:15") — a bare `Day N` counter (`TimeControlsView.gd`) is implemented; the countdown-to-nightfall half needs Phase 5.1's day/night split to exist first.
  - [ ] Threat Meter indicator per sector showing horde attraction levels, visible on the hex overview map — blocked on Phase 5.2's `HordeManager` existing to have a level to show.
  ...
  - [ ] Always-visible minimap of the Strategic layer (including Phase 2.7's markers/alerts) while zoomed into Tactical view — the hard-cut zoom (Phase 2.5) otherwise leaves the player with zero awareness of anything happening elsewhere on the map while zoomed into one hex cluster. Not implemented — pure QoL, not a playability blocker like the build menu was.
