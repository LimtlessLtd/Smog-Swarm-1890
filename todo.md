...
`design_doc.md` (v4.4) is now the authoritative spec for biome/terrain movement rules, the resource economy, the building tree, tech unlock thresholds, ZoC/field-supply rules, and the unit roster — it supersedes the equivalent detail that used to live in this file. Concrete reconciliation work needed before/while implementing it:
- **Resource economy rework.** Current `ResourceManager.gd` is 8 flat pools (Food, Energy, Gunpowder, Research, Wood, Bricks, Cast Iron, Reinforced Concrete — the last two currently gap-produced, no real recipe). `design_doc.md` §2 specifies a real raw→processed chain (6 raw + 5 processed + 4 capacity/yield types) with named recipes per processed resource. This is a rework of `ResourceManager`/`BuildingCatalog`, not an addition.
- **Building tree rework.** Current `BuildingCatalog.gd`'s 21 buildings (Housing & Civil / Industry & Extraction / Agriculture / Defense Works categories, e.g. Timber Camp, Clay Brickworks) need reconciling against `design_doc.md` §3's tier-0-to-5 building list (Lumber Yard, Clay Pit, Brickworks, etc. — different names, costs, and a real tier-gated unlock structure tied to Research thresholds).
