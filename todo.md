...
- [ ] **2.7.5 Under-Attack Alerts** — blocked on Phase 4.1/4.2 (wall breach) and Phase 5.4 (`CombatEngine` engagement events). Raises a pulsing alert at the specific wall segment or hex under attack (Phase 4.1's per-segment HP makes this precise rather than just "the district" generically), cleared once the threat resolves.
...
- [ ] **6.1 Background Play UI & HUD Design (`MainHUD.tscn`)**
  ...
  - [ ] Threat Meter indicator per sector showing horde attraction levels, visible on hex overview map.
...
- [ ] **7.2 Major Milestone Objectives System (`CampaignManager.gd`)**
  ...
  - [ ] **Secondary Objective:** Investigate regional relic sites (wrecked observatories, strange craters, underground testing facilities, captured UFO's etc.) revealing alien spore origins.
  - [ ] **Final Mystery:** Uncover the fate of Queen Victoria and the Imperial Cabinet inside the sealed Tower of London bunker. Did they know the alien invasion was coming this whole time? Leave it open ended for an expansion.
